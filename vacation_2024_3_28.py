...
st.markdown(
    ...
    "<div class='caption' style='text-align: right;'> 개발자 : 양산시 ChatGPT 학습동아리 팀</div>", 
    unsafe_allow_html=True)
st.title("🛫양산시 자기계발휴가 확인 서비스")
st.subheader('', divider='rainbow')
# txt = st.text_area(
#     "",
#     "1. 일자 입력으로 자기계발휴가를 확인할 수 있습니다.\n"
#     "2. 재직기간별 최초 사용시 사용 3일 전까지 '행정과'에 복무사항으로 보고 하여야합니다.\n"
#     "3. 차세대 인사랑시스템에 특별휴가 등록이 완료된 후 사용할 수 있습니다.\n"
#     "4. 자기계발휴가는 소급 및 이월 사용이 불가합니다.\n"
#     "5. 차세대 인사랑시스템의 '인사요약카드'에서 발령일자 등 확인하시면 됩니다."
#     ,height=140
#     )

st.markdown("""▪️ 재직기간별 자기계발휴가 일수""")
html_table = """
<table style='width: 100%; border-collapse: collapse;' border='1'>
    <tr>
        <th style='text-align: center;'>5년이상 10년미만</th>
        <th style='text-align: center;'>10년이상 20년미만</th>
        <th style='text-align: center;'>20년이상 30년미만</th>
        <th style='text-align: center;'>30년이상</th>
    </tr>
    <tr>
        <td style='text-align: center;'> 5일(1차 휴가)</td>
        <td style='text-align: center;'>20일(2차 휴가)</td>
        <td style='text-align: center;'>20일(3차 휴가)</td>
        <td style='text-align: center;'>20일(4차 휴가)</td>
    </tr>
</table>
"""
st.markdown(html_table, unsafe_allow_html=True)
st.markdown('')
st.caption("✔️ 차세대 인사랑시스템의 '인사요약카드'에서 최초임용일 등 일자를 확인할 수 있습니다.")
...
with col1:
    appointment_date_str = st.text_input("▪️ 최초임용일", key="appointment", 
                                     placeholder="20240315 형식 날짜 입력 후 엔터") 
with col2:    
    ...
    if appointment_date_str:
        ...
        if appointment_days is not None:
            ...
            #st.session_state.total_day += appointment_days
            st.markdown("")
            st.markdown("")
            st.write(f"🏆{formatted_today_date}까지 재직일수는 {appointment_days}일 입니다.")
            #st.write(f"{st.session_state.total_day}")
        else:
            st.markdown("")
            st.markdown("")
...
with col5:
    ...
    if military_service_start_date_str and military_service_end_date_str:
        ...
        if military_service_days is not None:
            ...
            st.markdown("")
            st.markdown("")
            st.write(f"🔰군대 경력은 총 {military_service_days}일로 재직일수에 합산됩니다.")
        else:
...
# 질병 휴직 등 경력 기간 계산 및 표시
    if sickness_start_date_str and sickness_end_date_str:
        ...
        if sickness_service_days is not None:
            ...
            st.markdown("")
            st.markdown("")
            st.markdown(f"🏣질병일수 {sickness_service_days}일은 재직일수에서 제외됩니다.")
        else:
...
with col9:
    parental_leave_start_date_str = st.text_input("▪️ 첫째 육아휴직 시작일",placeholder="YYYYMMDD", key="parental_start")
with col10:
    parental_leave_end_date_str = st.text_input("▪️ 첫째 육아휴직 종료일",placeholder="YYYYMMDD", key="parental_end")
with col11:
...
# 육아휴직 기간 계산 및 표시
    if add_parental_leave_start_date_str and add_parental_leave_end_date_str:
        ...
        if 두번째일수 is not None:
            ...
            st.write(f"👪 육아휴직 총 기간은 {첫번째일수 + 두번째일수}일 입니다.")
        else:
            st.write("날짜 형식이 올바르지 않습니다. YYYYMMDD 형식으로 다시 입력해주세요.")

st.caption("✔️ 육아휴직자의 경력은 첫째 자녀에 한하여 365일을 초과된 일수는 경력에서 제외됩니다.")

if 첫번째일수 or 두번째일수:
    if st.button("육아휴직 적용"):
        합산결과 = 첫번째일수 + 두번째일수 

    if 합산결과 > 365:
        제외경력 = 합산결과 - 365
        total_days -= 제외경력
        st.write(f"첫째 자녀 육아휴직 경력기간은 최대 365일이므로, {제외경력}일은 재직일수에서 제외되었습니다.")
    else:
        st.write("'육아휴직 적용' 버튼으로 재직일수를 반영하시기 바랍니다.")
...
if appointment_days:
    ...
    if error_message:
        ...
        # 결과 출력
        st.subheader('', divider='rainbow')
        st.write(f"👉 산정된 재직일수는 {total_days}일이며, 재직기간은 {years}년 {months}월 {days}일 입니다.")
        
        # 안식휴가 대상 판단
        if years < 5:
            # 5년 미만 직원의 경우
            d_day = 1825 - total_days  # 5년 차까지 남은 일수 계산
            future_date = today_date + timedelta(days=d_day)
            formatted_future_date = future_date.strftime("%Y년 %m월 %d일")
            st.subheader(f"🤐 1차 자기계발휴가 대상 아님.")
            st.subheader(f"🏖️ 1차 자기계발휴가 {formatted_future_date} 부터 사용가능.")
        elif 5 <= years < 10:
            d_day = 3650 - total_days #다음날 소멸로 1 플러스
            future_date = today_date + timedelta(days=d_day)
            formatted_future_date = future_date.strftime("%Y년 %m월 %d일")
            st.subheader(f"🥂 1차 자기계발휴가 5일 사용가능.")
            st.subheader(f"🤐 1차 자기계발휴가는 {formatted_future_date} 소멸 됩니다.")
        elif 10 <= years < 20:
            d_day = 7300 - total_days 
            future_date = today_date + timedelta(days=d_day)
            formatted_future_date = future_date.strftime("%Y년 %m월 %d일")         
            st.subheader(f"⛺ 1차 종료, 2차 자기계발휴가 20일 사용가능.") 
            st.subheader(f"🤐 2차 자기계발휴가는 {formatted_future_date} 소멸 됩니다.")
        elif 20 <= years < 30:
            d_day = 10950 - total_days 
            future_date = today_date + timedelta(days=d_day)
            formatted_future_date = future_date.strftime("%Y년 %m월 %d일")         
            st.subheader(f"🏝️ 1차 2차 종료, 3차 자기계발휴가 20일 사용가능.")
            st.subheader(f"🤐 3차 자기계발휴가는 {formatted_future_date} 소멸 됩니다.")
        else:  # 30년 이상
            st.subheader("🤐 1차, 2차, 3차 자기계발휴가는 종료.")
            st.subheader("👨‍👨‍👧‍👧 4차 안식휴가를 사용 할 수 있습니다.")